...
/**
 * @brief  配置USART端口.
 * @param  COM: 需要配置USART端口.
 *          参数可选值:    
 *            @arg COM1
 * @param  USART_InitStruct: USART_InitTypeDef结构体指针，该结构体中包含了所选中的
 *   				USART端口的配置信息.
 * @retval 无
 */
void STM_EVAL_COMInit(COM_TypeDef COM, USART_InitTypeDef* USART_InitStruct)
{
        ...
        /* 配置PA9端口为备注功能USART1_Tx */
        GPIO_PinAFConfig(COM_TX_PORT[COM], COM_TX_PIN_SOURCE[COM], COM_TX_AF[COM]);

        /* 配置PA10端口为备注功能USART1_Rx */
        GPIO_PinAFConfig(COM_RX_PORT[COM], COM_RX_PIN_SOURCE[COM], COM_RX_AF[COM]);

        /* 配置USART1_Tx端口属性 */
        GPIO_InitStructure.GPIO_Pin = COM_TX_PIN[COM];//PA9端口为USART1_Tx
        ...
        GPIO_Init(COM_TX_PORT[COM], &GPIO_InitStructure);//初始化GPIO
